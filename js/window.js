...
function asset_panel_template(prompt) {
    return `
        ...
        <div class="x-row">
            <div id="download-button1" class="modelviewer-panel-button enabled" onclick="downloadGLB(1)">Original GLB</div>
            <div id="download-button2" class="modelviewer-panel-button enabled" onclick="downloadGLB(2)">Edited GLB</div>
        </div>
    `;
}
...
function modelviewer_window_template(item1, item2, panel, config) {
    ...
    let html = '';
    html += `<div class="x-row" style="align-items: stretch; flex-wrap: nowrap; width: ${viewer_size * 2 + panel_size + 32}px; max-width: calc(100vw - 32px);">`
    ...
    html += `    </model-viewer>
        ...
        <div class="modelviewer-panel" style="flex: 0 0 ${panel_size}px; max-width: ${panel_size}px;">
            ${panel}
        </div>
    </div>`;
    return html;
